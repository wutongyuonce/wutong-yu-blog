...
- `src/components/views/*` 负责把内容集合转换成页面结构，例如博客列表、文章详情、项目列表、Insights 时间线
- 例如 `/blogs/` 页面会调用列表视图，而 `/insights/` 页面会调用 `InsightsView.astro` 读取 `insights` 集合并按规则渲染
...
| `insights` | `src/content/insights/**/*.md(x)` | Insights 时间线内容     |
...
│   ├── JUC并发编程/
│   │   ├── JUC 并发编程 1—多线程基础.md
│   │   ├── JUC 并发编程 2—Java 内存模型与底层同步机制.md
│   │   ├── JUC 并发编程 3—JUC 核心组件与并发实践（一）.md
│   │   └── JUC 并发编程 4—JUC 核心组件与并发实践（二）.md
│   ├── Vercel快速部署网站+国内访问.md
│   ├── markdown-guide.md
│   ├── 我的Mac配置清单.md
│   └── 我的vibe coding流程.md
├── home/
...
| `src/content/insights/年份/*.md`                               | Insights 时间线内容源，按年份分组进入页面渲染                 |
...
### 8.6 `src/content/insights/年份/*.md(x)`
...
它们最终都会进入 `insights` collection，并在页面中按 `pubDate` 倒序、按年份分组后输出为时间线。
...
getGroupedInsightsByYear('insights')
        ↓
InsightsView.astro 渲染为按年份分组的时间线
```
...
- `getGroupedInsightsByYear()`：按年份分组 `Insights` 时间线数据
...
- 渲染主内容区
- 在非首页显示 `BackLink`
- `/insights` 页面会为底部 `BackLink` 单独应用更宽的容器宽度，使 `cd ..` 与年份模块左边界对齐
- 渲染 Footer
- 渲染回顶按钮
- 渲染 `Backdrop`
...
| `InsightsView.astro` | 渲染 Insights 年份时间线 |
...
### 15.8 `InsightsView.astro`

这是 `Insights` 页面的核心视图组件。

它负责：

- 调用 `getGroupedInsightsByYear('insights')` 获取按年份分组的数据
- 对每条 Insight 执行 `render(item.entry)`，得到 Markdown 正文组件
- 先对 Markdown 正文做纯文本清洗，并生成前 200 字预览
- 超过 200 字时，收起态显示预览正文，展开后切换为完整正文
- 用独立状态控件控制每个年份列表的展开与收起
- 渲染年份头部、时间线节点、出处行与可选图片
- 标题使用手写字体，正文与展开态列表统一使用 `SourceHanSerifCN-Regular`
- 标题字号、正文字号与行高都通过 CSS 变量统一控制，并在小屏下单独覆盖
...
- 当前页面 frontmatter 使用 `bgType: snow`
- 通过 `BaseLayout + StandardLayout` 组装页面骨架
- 在 `StandardLayout` 内部包一层自定义 `article` 容器，单独放宽正文区域
- 挂载 `InsightsView.astro` 作为页面主体
- 底部 `cd ..` 返回链接由 `BaseLayout` 按 `/insights` 路由单独放宽容器宽度，并与年份起始线对齐
...
### 16.8 `src/pages/app.webmanifest.js`
...
- 会按 `pubDate` 参与倒序排序
- 会自动归入对应年份
- 超过 200 字时，收起态只显示前 200 字预览，展开后切换为完整正文
- 如果不提供 `image`，节点会自动切换为单列文本布局
...
- 静态页面共 13 个
- `/`、`/blogs/`、`/blogs/*`、`/projects/`、`/insights/`、`/friends/`、`/404.html`、`/app.webmanifest` 都已正常生成
...
- 当前只有带 `data-pagefind-body` 的页面正文会进入索引
- 搜索配置只包含 `blogs`
- `filter: false`
- 当前博客详情页会通过 `RenderPost.astro` 输出可搜索正文，因此实际被索引的是博客文章页
